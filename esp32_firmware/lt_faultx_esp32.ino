...
// ==================== CONFIGURATION ====================
// WiFi
const char *WIFI_SSID = "SONY Cable";
const char *WIFI_PASSWORD = "8593@9478";
...
const char *DEVICE_API_KEY =
    "f7a3b2c1d4e5f6a7b8c9d0e1f2a3b4c5"; // Must match backend DEVICE_API_KEY
...
// Fault Detection Thresholds (CONFIGURABLE - can be overridden by backend
// config)
...
float FAULT_RECOVERY_DELAY_MS =
    5000;                           // ms - confirm recovery after this duration
...
// Relay Safety
const int MIN_RELAY_SWITCH_INTERVAL_MS =
    5000; // Minimum time between relay switches

// ==================== STATE ====================
enum LineState { STARTING, HEALTHY, FAULT, ISOLATED, SENSOR_ERROR };

LineState currentLineState = STARTING;
LineState previousLineState = STARTING;
unsigned long stateChangeTime = 0;
bool relayState =
    true;             // true = CLOSED (line connected), false = OPEN (isolated)
...
void pollCommands();
void fetchConfig();
bool executeCommand(const String &commandId, const String &command,
                    JsonObject params);
void acknowledgeCommand(const String &commandId, const String &status,
                        bool resultState, const String &message);
...
void setup() {
  ...
  // START CLOSED so the line is powered and readings are valid
  digitalWrite(PIN_RELAY, LOW); // Start with relay CLOSED (line powered)
  relayState = true; // Track actual relay contact state: false = OPEN, true = CLOSED
  ...
  // Wait for sensors to settle before starting fault detection
  Serial.println("Stabilizing sensors...");
  for (int i = 0; i < 50; i++) {
    readSensors();
    delay(100);
  }
  Serial.printf("Startup readings: V=%.2fV I=%.3fA\n", filteredVoltage, filteredCurrent);
...
void loop() {
  ...
  if (now - lastStateDebug > 5000) {
    lastStateDebug = now;
    Serial.printf("[STATE] V=%.2fV I=%.3fA Relay=%s Line=%s Fault=%s\n",
                  filteredVoltage, filteredCurrent,
                  relayState ? "CLOSED" : "OPEN",
                  getStateString(currentLineState),
                  (currentLineState == FAULT) ? "YES" : "NO");
  }
  ...
  if (relayCommand != 0 &&
      now - lastRelaySwitchTime >= MIN_RELAY_SWITCH_INTERVAL_MS) {
    switch (relayCommand) {
    case 1:
...
void readSensors() {
  ...
  if (millis() - lastDebug > 5000) {
    lastDebug = millis();
    Serial.printf("[SENSORS] ADC_V=%d ADC_I=%d | V_sensor=%.3fV V_post2=%.2fV | I_sensor=%.3fV I=%.3fA\n",
                  voltageRaw, (int)avgCurrentRaw, voltageSensor, filteredVoltage, currentVoltageSensor, filteredCurrent);
  }
}
...
LineState evaluateLineState(float voltage, float current) {
  ...
  bool voltageOk =
      (voltage >= MIN_HEALTHY_VOLTAGE && voltage <= MAX_HEALTHY_VOLTAGE);
  bool currentOk = (current >= MIN_HEALTHY_CURRENT);

  // A line break shows up as voltage below the healthy floor regardless of
  // relay state. Report it as FAULT so the dashboard can flag it even when
  // the relay has already been opened by auto-isolation.
  if (!voltageOk) {
    return FAULT;
  }
  ...
  // If relay is intentionally open (and voltage is ok), line is isolated
  if (!relayState) {
    return ISOLATED;
  }

  // Otherwise fault (current out of range while relay closed)
  return FAULT;
}
...
void handleFaultDetection(LineState newState) {
  ...
  if (newState != currentLineState) {
    ...
    if (currentLineState == FAULT &&
        (now - stateChangeTime >= FAULT_CONFIRM_DELAY_MS)) {
    ...
    } else if (currentLineState == HEALTHY &&
               (now - stateChangeTime >= FAULT_RECOVERY_DELAY_MS)) {
...
void updateLCD() {
  ...
  if (millis() - lastLcdUpdate < 1000)
    return; // Update LCD at most once per second
...
void sendTelemetry() {
  ...
  HTTPClient http;
  String url =
      String(BACKEND_BASE_URL) + "/api/devices/" + DEVICE_ID + "/telemetry";
...
void pollCommands() {
  ...
  HTTPClient http;
  String url =
      String(BACKEND_BASE_URL) + "/api/devices/" + DEVICE_ID + "/command";
  ...
  if (httpCode == 200) {
    ...
    if (!err) {
      ...
      if (commandId.length() > 0 && command.length() > 0) {
        ...
        acknowledgeCommand(commandId, executed ? "ACKNOWLEDGED" : "FAILED",
                           executed,
...
void acknowledgeCommand(const String &commandId, const String &status,
  ...
  HTTPClient http;
  String url = String(BACKEND_BASE_URL) + "/api/devices/" + DEVICE_ID +
               "/command/" + commandId + "/ack";
...
void fetchConfig() {
  ...
  HTTPClient http;
  String url =
      String(BACKEND_BASE_URL) + "/api/devices/" + DEVICE_ID + "/config";
